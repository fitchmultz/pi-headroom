...
- Carries the trailing tool batch that no model has consumed yet in the automatic handoff: call arguments, bounded result text, and the entry ids needed to recover the rest. Consumed batches and older assistant prose are still left out.
...
- Detects unsupported small-context configurations (fewer than 10,000 usable tokens) with an actionable message instead of rolling over every turn; read pages shrink to the remaining budget and unsafe pages are refused with the offset preserved.
...
- Fingerprints reminders by window, context size, and reserve; a model switch inside a window gets a fresh reminder and stale ones are filtered. Legacy `headroom-reminder` entries remain recognized.
